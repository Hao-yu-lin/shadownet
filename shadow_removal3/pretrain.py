...
class ShadowModel(object):
    def __init__(self, args):
        ...
        """ Other """
        exp_time = "pretrained_" + datetime.today().strftime("%m-%d-%H")
    ...
    def train(self):
        ...
        for step in range(self.iteration):
            ...
            try:
                shadow, _ = train_shadow.next()
            except:
                train_shadow = iter(self.train_loader)
                shadow, _ = train_shadow.next()
            
            try:
                sh_free, _ = train_shadow_free.next()
            except:
            ...
            fake_A_logit, fake_A_Dom_logit, _ = self.disGA(fake_A2A)
            fake_B_logit, fake_B_Dom_logit, _ = self.disGB(fake_B2B)
            ...
            fake_A_logit, fake_A_Dom_logit, _ = self.disLA(fake_A2A)
            fake_B_logit, fake_B_Dom_logit, _ = self.disLB(fake_B2B)
            ...
            G_recon_loss_A = self.L1_loss(fake_A2A, shadow)   
            G_recon_loss_B = self.L1_loss(fake_B2B, sh_free)
            
            
            G_dom_loss_A = self.BCE_loss(fake_A2A_Dom_logit, torch.ones_like(fake_A2A_Dom_logit).to(self.device)) + self.BCE_loss(fake_A2A_Dom_logit, torch.zeros_like(fake_A2A_Dom_logit).to(self.device)) ##fake_A, 1; same_A(fake_A2A) 0
            G_dom_loss_B = self.BCE_loss(fake_B2B_Dom_logit, torch.ones_like(fake_B2B_Dom_logit).to(self.device)) + self.BCE_loss(fake_B2B_Dom_logit, torch.zeros_like(fake_B2B_Dom_logit).to(self.device)) ##fake_B, 1; same_B(fake_B2B) 0
            ...
            G_loss = loss_D + G_cycle_loss + G_dom_loss
            ...
            if (step % self.print_freq == 0) or (step == self.iteration-1):
                ...
                for idx, batch in enumerate(tqdm(self.test_loader)):
                    imgs, _ = batch
                    with torch.no_grad():
                        outputs, _, _ = self.genB2A(imgs.to(self.device))
                        outputs2, _, _ = self.genA2B(imgs.to(self.device))
                    ...
                    concat_img = np.concatenate((concat_img, RGB2BGR(tensor2numpy(self.denorm(outputs2[0])))), axis=1)
                ...
                torch.save(params, os.path.join(self.ckpt_folder, f'pretrained.pt'))
