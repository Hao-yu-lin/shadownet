...
class DCShadowNet(object):
    def __init__(self, args):
        ...
        self.train_output = "/home/haoyu/Desktop/partical/shadow_removal/output/train_img"
        ...
        exp_time = "2" + datetime.today().strftime("%m-%d-%H")
    ...
    def train(self):
        ...
        summary(self.disGA, (3, 256, 256), batch_size=1)
        ...
        # param = torch.load("/home/haoyu/Desktop/partical/shadow_removal/output/ckpt/02-15-18/_params_500.pt")
        ...
        for step in range(self.iteration):
            ...
            try:
                shadow = train_shadow.next()
            except:
                train_shadow = iter(self.train_loader)
                shadow = train_shadow.next()
            
            try:
                sh_free = train_shadow_free.next()
            except:
                train_shadow_free = iter(self.train_sh_free_loader)
                sh_free = train_shadow_free.next()
            ...
            G_loss = G_ad_loss * 2 + G_recon_loss * 15 + G_identity_loss * 15 + G_dom_loss
            ...
            # if(step <= 7049):
            #     concat_img1 = np.concatenate((RGB2BGR(tensor2numpy(denorm(shadow[0]))), RGB2BGR(tensor2numpy(denorm(sh_free[0])))), axis = 1)
            #     concat_img2 = np.concatenate((RGB2BGR(tensor2numpy(denorm(fake_A2B[0]))), RGB2BGR(tensor2numpy(denorm(fake_B2A[0])))), axis = 1)
            #     concat_img3 = np.concatenate((concat_img1, concat_img2))
                
            #     cv2.imwrite(os.path.join(self.train_output, f"{step}.png"), concat_img3 * 255.0)
            #     concat_img1 = None
            #     concat_img2 = None
            #     concat_img3 = None
                ...
                with open(os.path.join(self.ckpt_folder, "loss.txt"), "a") as the_file:
                    ...
                    the_file.write(
                        "[%5d/%5d] ch_loss: %.8f, G_A_loss: %.8f, G_B_loss: %.8f, G_loss: %.8f \n" 
                            % (step, self.iteration, 0, avg_G_A_loss, avg_G_B_loss, avg_G_loss)
